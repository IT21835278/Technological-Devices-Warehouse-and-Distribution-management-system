...
function EditstaffPayroll() {
    ...
    useEffect(() => {
      userRequest.get('/staffpayroll/' + id)
      .then(res => {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault()
        ...
        userRequest.put("/staffpayroll/" + id, {staffId,role,month,basic,otHours,otRate,otTotal,salary })
        .then(res => {
            toast.success('Payroll Deatils updated')
            navigate('/admin/payroll/ManagestaffPayroll')
        }).catch(err => {
            toast.error(err.message)
        })
