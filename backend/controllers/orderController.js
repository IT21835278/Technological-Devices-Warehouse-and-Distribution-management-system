...
const ChangeOrderStatus = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Order conform successful successfully', order });
  } catch (error) {
    res.status(500).json({ message: 'Error Conform order' });
  }
...
const CancelOrder = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const cancelledOrder = new CancelledOrder({
      username: orderToCancel.user.name,
      UserId: orderToCancel.user._id,
